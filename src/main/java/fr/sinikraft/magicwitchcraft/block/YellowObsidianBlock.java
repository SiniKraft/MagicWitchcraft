...
public class YellowObsidianBlock extends MagicWitchcraftModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("yellowobsidian", "yellowobsidian", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == GreyObsidianBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 8), Placement.COUNT_RANGE, new CountRangeConfig(16, 0, 0, 256)));
		}
	}
}
