...
public class SpectralChestOnBlockRightClickedProcedure extends MagicWitchcraftModElements.ModElement {
	public SpectralChestOnBlockRightClickedProcedure(MagicWitchcraftModElements instance) {
		super(instance, 229);
	}
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (!world.getWorld().isRemote) {
			world.playSound(null, new BlockPos((int) x, (int) y, (int) z),
					(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("magic_witchcraft:honey_pops")),
					SoundCategory.NEUTRAL, (float) 1, (float) 1);
		} else {
			...
			if (_ent instanceof ServerPlayerEntity) {
				...
				NetworkHooks.openGui((ServerPlayerEntity) _ent, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("SpectralChestInterface");
					}

					@Override
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						return new SpectralChestInterfaceGui.GuiContainerMod(id, inventory, new PacketBuffer(Unpooled.buffer()).writeBlockPos(_bpos));
					}
				}, _bpos);
			}
		}
