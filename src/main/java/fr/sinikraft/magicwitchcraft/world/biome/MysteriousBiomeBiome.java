...
public class MysteriousBiomeBiome extends MagicWitchcraftModElements.ModElement {
	@ObjectHolder("magic_witchcraft:mysteriousbiome")
	public static final CustomBiome biome = null;
	public MysteriousBiomeBiome(MagicWitchcraftModElements instance) {
		super(instance, 98);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.FOREST).waterColor(-14329397).waterFogColor(-14329397).parent("forest")
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(MysteriousLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(MysteriousLeavesBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MysteriousLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MysteriousLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MysteriousLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MysteriousLogBlock.block.getDefaultState().getBlock()
					|| blockType == MysteriousLeavesBlock.block.getDefaultState().getBlock()
