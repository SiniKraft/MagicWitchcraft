...
import fr.sinikraft.magicwitchcraft.procedure.ProcedureEnableFloatBootsOnKeyPressed;
import fr.sinikraft.magicwitchcraft.MagicWitchcraft;
import fr.sinikraft.magicwitchcraft.ElementsMagicWitchcraft;

@ElementsMagicWitchcraft.ModElement.Tag
public class KeyBindingEnableFloatBoots extends ElementsMagicWitchcraft.ModElement {
	private KeyBinding keys;
	public KeyBindingEnableFloatBoots(ElementsMagicWitchcraft instance) {
		super(instance, 162);
	}
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.enablefloatboots", Keyboard.KEY_BACK, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (Minecraft.getMinecraft().currentScreen == null) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				MagicWitchcraft.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			ProcedureEnableFloatBootsOnKeyPressed.executeProcedure($_dependencies);
		}
	}
}
